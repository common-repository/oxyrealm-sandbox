...
class Update {
	...
	public function __construct( $module_id, array $payload ) {
		$this->payload   = $payload;
		$this->module_id = $module_id;
	}

	public function ignite() {
		return new Updater( $this->payload['store_url'], $this->payload['plugin_file'], $this->payload );
	}

	public function isActivated() {
		$license = get_transient( "{$this->module_id}_license_seed" );

		if ( ! $license && empty( $this->payload['license'] ) ) {
			if ( array_key_exists( 'is_require_license', $this->payload ) && $this->payload['is_require_license'] ) {
				Notice::error( 'Enter your license key to get update', $this->module_id );
			}
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			Notice::error( is_wp_error( $response ) ? $response->get_error_message() : 'An Updater error occurred, please try again.', $this->module_id );
		...
		if ( $license_data->success === false ) {
			if ( property_exists( $license_data, 'error' ) ) {
				Notice::error( self::errorMessage( $license_data->error ), $this->module_id );
			}

			return false;
		}

		set_transient( "{$this->module_id}_license_seed", $license_data, 60 * 60 * 24 );
	...
	public function deactivate() {
		delete_transient( "{$this->module_id}_license_seed" );
